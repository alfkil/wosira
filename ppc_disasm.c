...
extern ULONG *RelocAdr, *RelocVal, nextreloc;
extern char adrbuf[64];
extern void GetLabel(LONG, UWORD);

extern ULONG *CodeArea1, *CodeArea2, *CodeAreaType, CodeAreas;
#define AREATYPE_WOS 2
#define AREATYPE_TOC 3

extern FILE *targetfile;
// **********
//**** illegal instructions might be pointers
// *** therefore we need a little more work here
// ***********
static void ill(struct DisasmPara_PPC *dp,ppc_word in)
{
  strcpy(dp->opcode,".long");
  if(RelocAdr[nextreloc] == (ULONG)dp->iaddr)
  {
	ULONG relocval = RelocVal[nextreloc];
	adrbuf[0] = '\0';
	GetLabel(relocval,9999);
	nextreloc++;
	sprintf(dp->operands, "%s", adrbuf);
...
static void addi(struct DisasmPara_PPC *dp,ppc_word in,char *ext)
{
  ...
  int i = (int)(in & 0xffff);
	  ...
	  if((int)PPCGETA(in) == 2 && hastoc)
		sprintf(dp->operands, "r%d,r2,@_toc%04x-@_toc0000",(int)PPCGETD(in),(short)i);
	  else
    	imm(dp,in,1,0);
  }
}
...
static void bc(struct DisasmPara_PPC *dp,ppc_word in)
{
  ...
  {
	if(dp->useLabels)
		sprintf(oper,"lab_%08lx",(unsigned long)((char *)dp->iaddr + d));
	else
...
static void bli(struct DisasmPara_PPC *dp,ppc_word in)
{
  ...
  {
	if (dp->useLabels)
		sprintf(dp->operands, "lab_%08lx",(unsigned long)((char *)dp->iaddr + d));
	else
...
//do toc offsets:
  if(a == 2 && hastoc)
	sprintf(dp->operands,"%c%d,@_toc%04x(r%d)",reg,s,d,a);
  else
...
ppc_word PPC_GetLabels(struct DisasmPara_PPC *dp)
{
  ...
  *(dp->operands) = 0;
  ...
  switch (PPCGETIDX(in)) {
    ...
    case 16:
	{
		...
		if (in & 2)  /* AA ? */
			return (ppc_word)0xffffffff;
		else
			return (ppc_word)((char *)dp->iaddr + d);
	}
    break;
